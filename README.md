# Esuopi Harri-Noel Ayamba | Portfolio

This repository contains the source code for my **personal portfolio website**, built to showcase my **projects, skills, certifications, and career interests** in software engineering and cybersecurity.

🌐 **Live Portfolio**  
👉 https://harri-ayamba.github.io/Portfolio/

---

## 👨‍💻 About Me
I am a **Software Engineering student** with a strong foundation in **HTML and CSS** (Cisco certified).  
I enjoy building clean, responsive web interfaces and I am actively growing toward **cybersecurity and DevSecOps**, with a strong interest in secure application development and Linux-based workflows.

---

## 🛠️ Technologies Used
- HTML5  
- CSS3  
- Responsive Web Design  
- CSS Animations  
- Git & GitHub  
- Linux (Zorin OS)

---

## 📁 Features
- Responsive hero section with smooth animations  
- Projects showcase  
- About section describing my learning path  
- Downloadable / viewable CV (PDF)  
- Contact links (LinkedIn, GitHub)

---

## 📄 Certifications
- **Cisco Certified – HTML & CSS**

---

## 🚀 Deployment
This portfolio is hosted using **GitHub Pages**.

---

## 📫 Contact
- **LinkedIn:** https://www.linkedin.com/in/esuopi-harri-noel-ayamba-aa5b0139a/#:~:text=www.linkedin.com/in/esuopi%2Dharri%2Dnoel%2Dayamba%2Daa5b0139a
- **GitHub:** https://github.com/harri-ayamba  

---

## 📌 Notes
This portfolio will be continuously improved as I build more projects and deepen my skills in **frontend development, cybersecurity, and DevSecOps**.
